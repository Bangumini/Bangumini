...
export type CollectionTaskKind =
	| "set-collection-type"
	| "set-progress"
	| "complete-progress";
...
export type SetCollectionTypeTask =
	PersistentCollectionTask<SetCollectionTypeTaskPayload> & {
		kind: "set-collection-type";
		status: PersistentCollectionTaskStatus;
	};

export type SetProgressTask =
	PersistentCollectionTask<SetProgressTaskPayload> & {
		kind: "set-progress";
		status: PersistentCollectionTaskStatus;
	};

export type CompleteProgressTask =
	PersistentCollectionTask<CompleteProgressTaskPayload> & {
		kind: "complete-progress";
		status: PersistentCollectionTaskStatus;
	};

export type CollectionTask =
	| SetCollectionTypeTask
	| SetProgressTask
...
function emitCollectionTaskQueueChanged() {
	window.dispatchEvent(new CustomEvent(COLLECTION_TASK_QUEUE_EVENT));
}

function isNotFoundError(error: unknown) {
	return (
		error instanceof Error && error.message.includes("Bangumi API error 404")
	);
}
...
function asCollectionTask(
	task: PersistentCollectionTask,
): CollectionTask | null {
	...
	if (
		...
		typeof payload.subjectId !== "number"
	)
		return null;
...
async function getTaskIdForEnqueue(
	kind: CollectionTaskKind,
	username: string,
	subjectId: number,
) {
...
function getExpectedNextType(task: CollectionTask): CollectionType | undefined {
	...
	if (task.kind === "set-progress")
		return task.payload.ensureWatching ? 3 : undefined;
	if (task.kind === "complete-progress")
		return task.payload.markWatched ? 2 : 3;
	return undefined;
}
...
function upsertCollection(
	...
	const idx = data.data.findIndex(
		(item) => item.subject_id === collection.subject_id,
	);
	if (idx >= 0) {
...
async function syncCollectionCaches(
	username: string,
	collection: UserCollection,
	previousType?: CollectionType,
) {
	await writeCachedCollection(username, collection);
	queryClient?.setQueryData(["collection", collection.subject_id], collection);

	const currentCacheKey = `${COLLECTIONS_CACHE_PREFIX}${collection.type}-${username}`;
	const currentCached =
		await readCachedValue<PagedResponse<UserCollection>>(currentCacheKey);
	if (currentCached?.data) {
		await writeCachedValue(
			currentCacheKey,
			upsertCollection(currentCached, collection),
		);
	}

	queryClient?.setQueryData<PagedResponse<UserCollection>>(
		["collections", String(collection.type), username],
		(old) => (old ? upsertCollection(old, collection) : old),
	);

	const typesToRemoveFrom =
		previousType && previousType !== collection.type
			? [previousType]
	...
	for (const type of typesToRemoveFrom) {
		...
		if (cached?.data) {
			await writeCachedValue(
				cacheKey,
				removeCollection(cached, collection.subject_id),
			);
		}

		queryClient?.setQueryData<PagedResponse<UserCollection>>(
			["collections", String(type), username],
	...
	queryClient?.invalidateQueries({
		queryKey: ["collection-status-map", username],
	});
}

async function fetchAndSyncCollection(
	username: string,
	subjectId: number,
	previousType?: CollectionType,
) {
...
function getMainEpisodeIdsBetween(
	episodes: Episode[],
	fromEp: number,
	toEp: number,
) {
...
async function executeProgressTask(
	payload: SetProgressTaskPayload | CompleteProgressTaskPayload,
) {
	const { username, subjectId, targetEp, ensureWatching, previousType } =
		payload;
	...
	if (currentEp !== targetEp) {
		...
		collection = await fetchAndSyncCollection(
			username,
			subjectId,
			collection.type,
		);
...
async function showFailureToast(task: CollectionTask) {
	const message =
		task.kind === "set-progress" || task.kind === "complete-progress"
			? "进度同步失败，将稍后重试"
			: "收藏状态同步失败，将稍后重试";
...
export async function enqueueSetCollectionTypeTask(
	payload: SetCollectionTypeTaskPayload,
) {
	const task = await upsertPersistentCollectionTask({
		id: await getTaskIdForEnqueue(
			"set-collection-type",
			payload.username,
			payload.subjectId,
		),
...
export async function enqueueCompleteProgressTask(
	payload: CompleteProgressTaskPayload,
) {
	const task = await upsertPersistentCollectionTask({
		id: await getTaskIdForEnqueue(
			"complete-progress",
	...
	try {
		while (true) {
			...
			try {
				...
				await markPersistentCollectionTaskFailed(
					task.id,
				...
				emitCollectionTaskQueueChanged();
				if (task.attemptCount === 0 || document.hasFocus()) {
					await showFailureToast(task);
				}
			}
		}
	} finally {
