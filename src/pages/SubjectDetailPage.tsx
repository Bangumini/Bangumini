...
function getAirWeekdayLabel(airWeekday?: number, date?: string) {
	...
	if (airWeekday && bangumiWeekdays[airWeekday])
		return bangumiWeekdays[airWeekday];
...
function extractArtist(
	...
	if (
		...
		(parts.length === 0 || composer.value !== parts[0])
	) {
...
function SubjectDetailContent({ subjectId }: { subjectId: number }) {
	...
	async function loadEpisodesFromCacheOrNetwork() {
		const cached = await readCachedEpisodesWithin(
			subjectId,
	...
	async function ensureEpisodesLoaded() {
		...
		const result =
			await queryClient.ensureQueryData<PagedResponse<Episode> | null>({
	...
	async function loadRelations(subjectId: number) {
		const cached = await readCachedRelationsWithin(
			subjectId,
			DETAIL_CACHE_MAX_AGE,
		);
		if (cached) return cached;
		...
		if (stale) {
			refreshQueryInBackground(queryClient, ["relations", subjectId], () =>
				getSubjectRelations(subjectId).then((r) =>
					writeCachedRelations(subjectId, r),
				),
			);
			return stale;
		}
	...
	const { data: subject } = useQuery({
		...
		queryFn: async () => {
			const cached = await readCachedSubjectDeepWithin(
				subjectId,
				DETAIL_CACHE_MAX_AGE,
			);
			if (cached) {
	...
	const { data: persons } = useQuery({
		...
		queryFn: async () => {
			const cached = await readCachedPersonsWithin(
				subjectId,
				DETAIL_CACHE_MAX_AGE,
			);
			if (cached) return cached;
	...
	const { data: characters } = useQuery({
		...
		enabled: loadSecondaryDetailData && Boolean(subject),
		queryFn: async () => {
			const cached = await readCachedCharactersWithin(
				subjectId,
				DETAIL_CACHE_MAX_AGE,
			);
			if (cached) return cached;
	...
	const { data: collection } = useQuery({
		...
		queryFn: async () => {
			...
			const cached = await readCachedCollectionWithin(
				uname,
				subjectId,
				DETAIL_CACHE_MAX_AGE,
			);
	...
	const sorted =
		episodeData?.data?.slice().sort((a, b) => a.sort - b.sort) ?? [];
	...
	const activeCollectionTask = subjectCollectionTasks.find(
		(task) => task.status === "pending" || task.status === "running",
	);
	const failedCollectionTask = subjectCollectionTasks.find(
	...
	function confirmMarkWatchedAfterSave(
	...
	useKeyboardShortcuts(
		[
			...
			},
			{
				...
				mod: true,
				handler: () => {
					const idx = COLLECTION_OPTIONS.findIndex(
						(o) => o.type === (currentColType ?? 3),
					);
				...
				<div className="w-72 shrink-0 border-l border-line p-5 flex flex-col gap-4 overflow-y-auto bg-panel/40">
					...
					<div className="space-y-3 text-[13px]">
						<div className="flex items-baseline gap-2">
							...
							{subject?.rank ? (
								<span className="text-fg-tertiary">#{subject.rank}</span>
							) : null}
						</div>
						...
						<div>
							...
							{currentColType ? (
								<span className="text-accent font-medium">
									{CollectionTypeLabel[currentColType]}
								</span>
							) : (
								<span className="text-fg-tertiary">未收藏</span>
							)}
							{activeCollectionTask && (
								<span className="text-fg-tertiary ml-1">同步中</span>
							)}
							{failedCollectionTask && (
								<span className="text-danger ml-1">同步失败</span>
							)}
						</div>
						...
						{totalEp > 0 && (
							<div className="pt-1">
								...
								{!isDirty && activeCollectionTask && (
									<p className="text-[12px] text-fg-tertiary mt-1.5">
										后台同步中，可返回或隐藏窗口
									</p>
								)}
								{!isDirty && failedCollectionTask && (
					...
					<div className="relative w-64 bg-elevated rounded-xl border border-line-strong shadow-pop overflow-hidden">
						...
						<div className="px-2 pb-1">
							{COLLECTION_OPTIONS.map((opt, i) => (
								...
								>
									<kbd
										className={`text-[11px] w-4 text-center ${i === paletteIndex ? "text-accent-fg/70" : "text-fg-tertiary"}`}
									>
										{opt.key}
									</kbd>
									{currentColType === opt.type && (
										<span
											className={`text-[11px] ${i === paletteIndex ? "text-accent-fg" : "text-accent"}`}
										>
											●
										</span>
									)}
