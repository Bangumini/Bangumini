...
export default function CalendarPage() {
	...
	const filteredItems = useMemo(() => {
		...
		const filtered = allItems.filter((item) => {
			if (filterWeekday && item.weekday !== parseInt(filterWeekday))
				return false;
			if (filterText) {
	...
	const displayItems = isFiltering
		? filteredItems
				...
				handler: () => {
					...
					if (item) {
						import("@tauri-apps/plugin-opener").then(({ openUrl }) => {
							openUrl(`https://bgm.tv/subject/${item.id}`);
						});
					}
				},
			},
	...
	if (isLoading && !calendar)
		return <p className="p-4 text-fg-tertiary text-[13px]">加载中…</p>;
	if (error && !calendar)
		return (
			<p className="p-4 text-danger text-[13px]">加载出错: {String(error)}</p>
		);
	if (!calendar || calendar.length === 0)
		return <p className="p-4 text-fg-tertiary text-[13px]">暂无放送数据</p>;
				...
				{isFiltering ? (
					...
					<>
						...
						<div className="flex items-center justify-between mb-2 px-1">
							<h2
								className={`text-[15px] font-semibold ${isToday ? "text-accent" : "text-fg"}`}
							>
								{WEEKDAY_CN[currentDay]}
								{isToday && " · 今天"}
								{currentDayData?.weekday.ja && (
									<span className="text-[12px] font-normal text-fg-tertiary ml-2">
										{currentDayData.weekday.ja}
									</span>
								)}
							</h2>
