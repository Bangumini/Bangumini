...
function isAired(
	item: Pick<NextSeasonItem, "airingAt" | "startDate">,
): boolean {
...
function isBangumiMatchCache(
	value: BangumiMatchCache | null,
): value is BangumiMatchCache {
	return (
		!!value &&
...
async function fetchBangumiMatch(
	item: NextSeasonItem,
): Promise<BangumiMatchCache> {
	const resp = await searchSubjects({
		keyword: item.title.native,
		type: [2],
		limit: 3,
	});
...
async function hydrateBangumiMatchCachesFromEntries(entries: SeasonEntry[]) {
	...
	await Promise.all(
		positiveMatches.map((entry) =>
			writeCachedValue(getNextSeasonBangumiMatchCacheKey(entry.id), {
...
async function migrateLegacyNextSeasonBaseCache(
	nextSeasonBaseCacheKey: string,
) {
	const legacySqlite = await readCachedValue<SeasonEntry[]>(
		getLegacySqliteNextSeasonCacheKey(),
	);
...
async function resolveNextSeasonEntries(
	...
	const cachedMatches = await Promise.all(
		baseItems.map((item) =>
			readCachedValueEntry<BangumiMatchCache>(
				getNextSeasonBangumiMatchCacheKey(item.id),
			),
		),
	);

	const matches: Array<BangumiMatchCache | null> = new Array(
		baseItems.length,
	).fill(null);
	const pendingMatches: Array<{
		index: number;
		item: NextSeasonItem;
		fallback: BangumiMatchCache | null;
	}> = [];
	...
	for (let index = 0; index < baseItems.length; index += 1) {
		...
		const payload = isBangumiMatchCache(cached?.payload ?? null)
			? cached!.payload
			: null;
		const stale = cached
			? isCacheStale(cached.updatedAt, NEXT_SEASON_BANGUMI_MATCH_TTL)
			: true;
		const titleChangedWithoutMatch =
			payload !== null &&
	...
	if (options.refreshMissingOrStaleMatches && pendingMatches.length > 0) {
		...
		pendingMatches.forEach((pending, index) => {
			matches[pending.index] =
				refreshedMatches[index] ?? matches[pending.index];
		});
...
export default function NextSeasonPage() {
	...
	const {
		...
		queryFn: async () => {
			...
			if (!cachedBase) {
				const migratedBase = await migrateLegacyNextSeasonBaseCache(
					nextSeasonBaseCacheKey,
				);
				if (migratedBase) {
					cachedBase = await readCachedValueEntry<NextSeasonItem[]>(
						nextSeasonBaseCacheKey,
					);
				}
			}
			...
			if (cachedBase) {
				...
				const staleBase = isCacheStale(
					cachedBase.updatedAt,
					NEXT_SEASON_BASE_CACHE_TTL,
				);
					...
					refreshQueryDataIfChanged({
						...
						refresh: async () => {
							const refreshedBase = staleBase
								...
								: baseItems;
							const nextResolved = await resolveNextSeasonEntries(
								refreshedBase,
								{
			...
			try {
				const baseItems = await fetchAndCacheNextSeasonBase(
					nextSeasonBaseCacheKey,
				);
				const resolved = await resolveNextSeasonEntries(baseItems, {
					refreshMissingOrStaleMatches: true,
	...
	const filteredGroups = useMemo(() => {
		...
		const items = entries.filter((item) => {
			...
			if (filterWeekday && String(item.weekday ?? "tba") !== filterWeekday)
				return false;
			if (filterText) {
	...
	return (
		<div className="h-full flex flex-col">
			...
			{isFiltering && (
				<div className="px-4 py-1.5 text-[12px] text-fg-tertiary border-b border-line shrink-0">
					<span>
						...
						{filterWeekday &&
							` · ${filterWeekday === "tba" ? "未定" : filterWeekday === "nontv" ? "非TV" : ANILIST_WEEKDAY_CN[parseInt(filterWeekday)]}`}{" "}
			...
			<div className="flex-1 overflow-y-auto p-2.5">
				{isLoading && !rawEntries && (
					<p className="text-fg-tertiary text-[13px] px-1">加载中…</p>
				)}
				{error && !rawEntries && (
					<p className="text-danger text-[13px] px-1 mb-2">
						加载出错: {String(error)}
					</p>
				)}
				{error && rawEntries && (
					<p className="text-fg-tertiary text-[12px] px-1 mb-2">
						加载失败，显示缓存数据
					</p>
				)}
