...
export default function Layout() {
	...
	const [filterPaletteKind, setFilterPaletteKind] =
		useState<FilterPaletteKind>("search");
			...
			return (
				<>
					...
					<CustomSelect
						options={SUBJECT_TYPES}
						value={stype}
						onChange={() => openFilterPalette("search")}
					/>
				</>
			);
		}
			...
			return (
				<>
					...
					<CustomSelect
						options={CALENDAR_WEEKDAYS}
						value={weekday}
						onChange={() => openFilterPalette("calendar")}
					/>
				</>
			);
		}
			...
			return (
				<>
					...
					<CustomSelect
						options={COLLECTION_TYPES}
						value={type}
						onChange={() => openFilterPalette("collections")}
					/>
				</>
			);
		}
			...
			return (
				<>
					...
					<CustomSelect
						options={NEXT_SEASON_WEEKDAYS}
						value={weekday}
						onChange={() => openFilterPalette("next-season")}
					/>
				</>
			);
		}
	...
	const {
		...
		currentValue: paletteCurrentValue,
		title: paletteTitle,
	} = getPaletteConfig(filterPaletteKind);
		...
		<div className="h-screen flex text-fg overflow-hidden">
			...
			<div className="flex-1 flex flex-col min-w-0">
				...
				<footer className="flex items-center gap-4 h-9 px-4 border-t border-line shrink-0 bg-panel/40">
					...
					{isCollections && <KeyHint k={`${MOD}+O`} label="浏览器打开" />}
					{isCalendar && <KeyHint k={`${MOD}+O`} label="浏览器打开" />}
					{isNextSeason && <KeyHint k={`${MOD}+O`} label="浏览器打开" />}
				</footer>
			</div>
							...
							{paletteOptions.map((opt, i) => (
								...
								>
									{opt.value === paletteCurrentValue && (
										<span
											className={`w-3.5 text-center text-[11px] ${i === filterPaletteIndex ? "text-accent-fg" : "text-accent"}`}
										>
											●
										</span>
									)}
